...
function formatAjvErrors (errors) {
  const stringErrors = []
  const handledKeywords = ['required', 'enum', 'type', 'pattern']
  ...
  // unhandled errors
  errors
    .filter((error) => !handledKeywords.includes(error.keyword))
    .forEach((error) =>
      stringErrors.push(`WARNING: keyword '${error.keyword}' was not handled for formatting: ${JSON.stringify(error, null, 2)}`)
    )
