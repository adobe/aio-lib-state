 ...
 *
 * @typedef StateStoreKeyObject
 * @type {object}
 * @property {string} key state key identifier
 * @property {number} ttl time to live in seconds
 * @property {number} timestamp key timestamp
 */

/**
 * StateStore list return object
 *
 * @typedef StateStoreListReturnObject
 * @type {object}
 * @property {object} StateStoreKeyObject with details related to State Store keys
 * @property {boolean} hasMoreResults flag to indicate whether there are more results
 * @property {string} continuationToken used to fetch next batch of results
 */
...
class StateStore {
   ...
   *
   * @param {string} continuationToken use in subsequent calls to get next set of keys in case of paginated results
   * @returns {Promise<Array<StateStoreListReturnObject>>} array of keys and additional info
   * @memberof StateStore
   */
  async list (continuationToken) {
    logger.debug(`list '${continuationToken}'`)
    return this._list(continuationToken)
  }
   ...
   * @protected
   */
  async _list (continuationToken) { throwNotImplemented('_list') }
}
