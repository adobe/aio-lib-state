...
class AdobeStateStore {
  ...
  async get (key) {
    ...
    if (!valid) {
      logAndThrow(new codes.ERROR_BAD_ARGUMENT({
        messageValues: ['invalid key or value', JSON.stringify(errors)],
  ...
  async _get (key) {
    ...
    logger.debug('_get', requestOptions)
  ...
  async _put (key, value, options) {
    ...
    logger.debug('_put', requestOptions)
  ...
  async _delete (key) {
    ...
    logger.debug('_delete', requestOptions)
    const promise = this.fetchRetry.exponentialBackoff(this.createRequestUrl(key), requestOptions)
  ...
  async _deleteAll () {
    ...
    logger.debug('_deleteAll', requestOptions)
  ...
  async _any () {
    ...
    logger.debug('_any', requestOptions)
    const promise = this.fetchRetry.exponentialBackoff(this.createRequestUrl(), requestOptions)
    const response = await _wrap(promise, {})
    return response !== null
  }
