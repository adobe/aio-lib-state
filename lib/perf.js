...
function createObserver (logger = console.log) {
  new PerformanceObserver((list, observer) => {
    ...
    .observe({ entryTypes: ['function', 'measure'], buffered: false })
}
...
/**
 * Measure the performance of a regular function.
 *
 * @param {string} tag the tag to use as a prefix for the performance measurement
 * @param {Function} fn the function to measure
 * @returns {object} the result from the function
 */
function perfMeasure (tag, fn) {
  const p = perfMark(tag)
  p.start()
  const retVal = fn()
  p.end()
