...
const ADOBE_STATE_STORE_ENDPOINT = {
  ...
  [STAGE_ENV]: 'http://localhost:8080' // TODO:
}
...
async function _wrap (promise, params) {
  ...
  try {
    ...
    logger.debug(`got internal error with status ${statusCode}: ${e.message} `)
    switch (statusCode) {
      case 401:
      ...
      default:
        return logAndThrow(new codes.ERROR_INTERNAL({ messageValues: [`unexpected response from provider with status: ${statusCode}`], sdkDetails: { ...cloneDeep(params), _internal: e.internal } }))
    }
  }
  return response
}
...
class AdobeStateStore extends StateStore {
   ...
   */
  async _get (key) {
    const promise = fetchRetry.exponentialBackoff(this.createRequestUrl(key), {
      method: 'GET',
      headers: {
  ...
  async _put (key, value, options) {
    ...
    const promise = fetchRetry.exponentialBackoff(this.createRequestUrl(key, { ttl }), {
      method: 'PUT',
      headers: {
        ...this.getAuthorizationHeaders(),
        'Content-Type': 'application/octet-stream'
      },
      body: fs.createReadStream(value)
    })
...
AdobeStateStore.DefaultTTL = 86400 // 24 hours (in seconds)
