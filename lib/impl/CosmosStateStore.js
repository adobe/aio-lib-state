...
async function _wrap (promise, params) {
  ...
  try {
    ...
    if (status === 403) throw new errorCodes.ERROR_BAD_CREDENTIALS({ messageValues: ['underlying DB provider'], sdkDetails: cloneDeep(params) })
    if (status === 413) throw new errorCodes.ERROR_PAYLOAD_TOO_LARGE({ sdkDetails: cloneDeep(params) })
    throw new errorCodes.ERROR_INTERNAL({ messageValues: [`unknown error response from provider with status: ${status || 'unknown'}`], sdkDetails: { ...cloneDeep(params), _internal: e } })
  }
...
class CosmosStateStore extends StateStore {
  ...
  static async init (credentials) {
    ...
    if (validation.error) {
      ...
      throw new errorCodes.ERROR_INTERNAL({
