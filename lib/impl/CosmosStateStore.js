...
async function _wrap (promise, key) {
  ...
  try {
    ...
    if (status === 413) throw new StateStoreError(`key, value or request payload is too large`, StateStoreError.codes.PayloadTooLarge)
...
class CosmosStateStore extends StateStore {
  ...
  static async init (credentials) {
    ...
    if (credentials.resourceToken) {
      cosmosClient = new cosmos.CosmosClient({ endpoint: credentials.endpoint, consistencyLevel: 'Session', tokenProvider: async () => credentials.resourceToken })
    } else {
  ...
  async _put (key, value, options) {
    ...
    await _wrap(this._cosmos.container.items.upsert({ id: key, partitionKey: this._cosmos.partitionKey, ttl, value }))
    return key
  }
