...
class CosmosStateStore extends StateStore {
   ...
   * @private
   */
  async _list (continuationToken) {
    const queryPlan = await _wrap(this._cosmos.container.items.query(`SELECT c.id,c.ttl,c._ts from c where c.partitionKey='${this._cosmos.partitionKey}'`, {
      initialHeaders: {
        'x-ms-documentdb-partitionkey': `["${this._cosmos.partitionKey}"]`
      },
      continuationToken: continuationToken
    }))
    const response = await _wrap(queryPlan.fetchNext())
    const keys = response.resources
      .filter((obj) => {
        return obj.id !== '__aio' // remove system generated key
      }).map((x) => {
        return { key: x.id, ttl: x.ttl, timestamp: x._ts }
      })
    const ret = {
      keys: keys,
    ...
    if (response.hasMoreResults) {
      ret.continuationToken = response.continuationToken
    }
    return ret
  }
}
