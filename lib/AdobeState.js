...
const { ADOBE_STATE_STORE_ENDPOINT, REGEX_PATTERN_STORE_KEY, API_VERSION } = require('./constants')
...
class AdobeState {
  ...
  createRequestUrl (key, queryObject = {}) {
    ...
    if (key) {
      requestUrl = new url.URL(`${protocol}://${region}${this.endpoint}/${API_VERSION}/containers/${this.namespace}/data/${key}`)
    } else {
      requestUrl = new url.URL(`${protocol}://${region}${this.endpoint}/${API_VERSION}/containers/${this.namespace}`)
    }
