...
const { REGEX_PATTERN_STORE_KEY, API_VERSION, HEADER_KEY_EXPIRES, CUSTOM_ENDPOINT, ENDPOINTS, ALLOWED_REGIONS, REGEX_PATTERN_LIST_KEY, MAX_LIST_COUNT_HINT, REQUEST_ID_HEADER, MIN_LIST_COUNT_HINT } = require('./constants')
...
async function _wrap (promise, params, throwsOn404 = false) {
  let response
  try {
  ...
  return handleResponse(response, params, throwsOn404)
}
 ...
 * @param {boolean} throwsOn404 errors out on 404
 * @returns {void}
 */
async function handleResponse (response, params, throwsOn404) {
  if (response.ok) {
    return response
  }
...
class AdobeState {
   ...
   * @example
   *  for await (const { keys } of state.list({ pattern: 'abc*' })) {
   *    console.log(keys)
   *  }
   * @param {object} options list options
   * @param {string} options.pattern a glob pattern that supports '*' to filter
   *   keys.
  ...
  list (options = {}) {
    ...
    const queryParams = {}
    if (options.pattern) {
      queryParams.pattern = options.pattern
    }
    ...
    const schema = {
      type: 'object',
      properties: {
        pattern: { type: 'string', pattern: REGEX_PATTERN_LIST_KEY }, // this is an important check
