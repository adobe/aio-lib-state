...
const { ADOBE_STATE_STORE_ENDPOINT, REGEX_PATTERN_STORE_KEY, API_VERSION, ADOBE_STATE_STORE_REGIONS } = require('./constants')
...
class AdobeState {
   ...
   * @param {string} [region] the region for the Adobe State Store
   */
  ...
  createRequestUrl (key, queryObject = {}) {
    ...
    if (key) {
      requestUrl = new url.URL(`${protocol}://${this.region}.${this.endpoint}/${API_VERSION}/containers/${this.namespace}/data/${key}`)
    } else {
      requestUrl = new url.URL(`${protocol}://${this.region}.${this.endpoint}/${API_VERSION}/containers/${this.namespace}`)
    }
