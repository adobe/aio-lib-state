 ...
 * @property {('amer'|'apac'|'emea'|'aus')} region the region for the Adobe State Store. defaults to 'amer'
 */
 ...
 *   to 24 hours.
 * @property {boolean} [ifNotExists] Only write the key if it does not already
 ...
 * @property {boolean} [ifExists] Only write the key if it already exists.
...
class AdobeState {
   ...
   * @param {('amer'|'apac'|'emea'|'aus')} [region] the region for the Adobe State Store. defaults to 'amer'
   ...
   * @returns {Promise<string|null>} key, or `null` if a specified ifNotExists/ifExists
  ...
  async put (key, value, options = {}) {
    ...
    // validation
    const { ttl, ifNotExists, ifExists } = options
    const { valid, errors } = validate(schema, { key, value, ttl, ifNotExists, ifExists })
    if (!valid) {
      logAndThrow(new codes.ERROR_BAD_ARGUMENT({
        messageValues: utils.formatAjvErrors(errors),
    ...
    if (ifNotExists && ifExists) {
      logAndThrow(new codes.ERROR_BAD_ARGUMENT({
        messageValues: 'ifNotExists and ifExists are mutually exclusive.',
        sdkDetails: { key, valueLength: value.length, options }
      }))
    }
    ...
    const response = await _wrap(promise, { key, value, ...options })
    if ((ifNotExists && response.status === 409) || (ifExists && response.status === 404)) {
      // the requested condition was not met, nothing was written
      return null
    }
    return key
  }
  ...
  list (options = {}) {
    ...
    // Warn if countHint is passed - it has no effect
    if (options.countHint !== undefined) {
      logger.warn('The \'countHint\' option has been removed and is ignored.')
    }
