...
const { AdobeStateStore } = require('./impl/AdobeStateStore')
...
async function init (config = {}) {
  ...
  // 1. get provider (default 'adobe')
  const { provider = 'adobe' } = config
  ...
  switch (provider) {
    ...
    case 'adobe':
      return AdobeStateStore.init({ apikey: config.adobe?.apikey })
    default:
      throw new codes.ERROR_UNKNOWN_PROVIDER(`provider '${provider}' is not supported.`)
  }
}
