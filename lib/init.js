...
// eslint-disable-next-line jsdoc/require-jsdoc
function withHiddenFields (config, fields) {
  ...
  fields.forEach(f => {
    ...
    if (traverse) traverse[lastKey] = '<hidden>'
  })
  return copyConfig
}
...
async function init (config = {}) {
  ...
  // 0. log
  const logConfig = withHiddenFields(config, ['ow.auth', 'cosmos.resourceToken', 'cosmos.masterKey'])
