...
const { perfAsyncMeasure, createObserver } = require('./perf')
...
async function init (config = {}) {
  ...
  createObserver((...args) => logger.debug(...args))
  ...
  if (provider === 'cosmos' && !config.cosmos) {
    ...
    tvm = await perfAsyncMeasure(
      'tvmclient-init',
      async () => TvmClient.init(tvmArgs)
    )
  }
  ...
  switch (provider) {
    case 'cosmos':
      if (config.cosmos) {
        ...
        return perfAsyncMeasure(
          'cosmos-creds-init',
          async () => CosmosStateStore.init(config.cosmos)
        )
      }
      logger.debug('init with openwhisk credentials')
      return perfAsyncMeasure(
        'ow-init',
        async () => CosmosStateStore.init(
          await perfAsyncMeasure(
            'tvm-wrap',
            async () => wrapTVMRequest(tvm.getAzureCosmosCredentials()))
        )
      )
    // default:
