...
const { ERROR_BAD_CREDENTIALS } = require('./StateStoreError').codes
  ...
  return promise
    .catch(e => {
      if (e.status === 401 || e.status === 403) {
        throw new ERROR_BAD_CREDENTIALS({ messageValues: ['TVM'], sdkDetails: cloneDeep(params) })
      }
...
async function init (config = {}) {
  ...
  switch (provider) {
    case 'cosmos':
      return CosmosStateStore.init(config.cosmos || (await wrapTVMRequest(tvm.getAzureCosmosCredentials(), { ...config.tvm, namespace: config.ow.namespace })))
    // default:
